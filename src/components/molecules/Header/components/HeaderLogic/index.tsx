...
export default function HeaderLogic({children}: {children: ReactNode}) {
    const { open } = useMenu()
    ...
    return (
        <header
            className={cn(
                'fixed z-50 w-full h-fit top-0 flex justify-center items-start transition-colors duration-500 rounded-b-[30px] overflow-hidden',
                isDown && 'bg-black'
            )}
        >
