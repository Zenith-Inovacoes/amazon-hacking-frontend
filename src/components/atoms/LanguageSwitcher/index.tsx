...
const LanguageSwitcher = forwardRef(
  (
    { className, ...props }: LanguageSwitcherProps,
    ...
    return (
      <SelectPrimitive.Root
        ...
        onValueChange={(value) => {
          router.push(`/${value}`);
          setCurrentLanguage(
            locales.find((locale) => locale.locale === value)!
          );
        }}
        {...props}
      >
          ...
          <SelectPrimitive.Value>
            <div className="flex items-center whitespace-nowrap">
              ...
              <SelectPrimitive.Icon
                asChild
                className="ml-2 group-data-[state=open]:visible opacity-0 group-hover:opacity-100 group-data-[state=open]:opacity-100 group-data-[state=open]:rotate-180 group-data-[state=closed]:rotate-0 transition-all duration-500"
              >
        ...
        <SelectPrimitive.Portal>
          <SelectPrimitive.Content
            className="min-w-[var(--radix-select-trigger-width)]  z-[9999]  shadow-languageSwitcher cursor-pointer"
            position="popper"
          >
            <SelectPrimitive.Viewport>
              {locales.map((locale, index) => (
                <SelectPrimitive.Item
                  key={index}
                  value={locale.locale}
                  className="font-normal text-base leading-4 px-4 py-4 bg-neutrals-600 focus-visible:outline-none hover:bg-neutrals-700 focus-visible:bg-neutrals-700 transition-colors data-[state=checked]:hidden"
                >
