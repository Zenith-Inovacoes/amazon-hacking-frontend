...
const Select = forwardRef(({ placeholder, children, variant = "dark", ...props }: SelectProps, ref: Ref<HTMLButtonElement>) => {
    return (
        <SelectPrimitive.Root {...props}>
            <SelectPrimitive.Trigger ref={ref} className={cn("flex w-full z-50 justify-between gap-4 px-4 py-3 items-center hover:bg-neutrals-300 hover:rounded-[10px] transition-all group border-b font-bold text-base leading-4 focus-visible:outline-none focus-visible:outline-1 focus-visible:outline-offset-4 focus-visible:outline-primary-100 data-[state=open]:bg-primary-100 data-[state=open]:rounded-[10px] data-[state=open]:border-transparent data-[state=open]:text-white", {
