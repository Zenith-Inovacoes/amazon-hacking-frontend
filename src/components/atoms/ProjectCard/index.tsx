...
export default function ProjectCard({ ...props }: ProjectCardProps) {
  ...
  const ytURL =
    locale === "en" ? props.links.youtube.en : props.links.youtube.pt;

  const childrenData = {
    solutionName: props.name,
    teamLogo: props.logoUrl || props.links.banner || LogoComputacaoAmostra,
    solutionDescription: props.description,
  ...
  return (
    <div className="group p-3 gap-x-4 hover:p-6 rounded-xl justify-center items-center bg-white transition-all duration-500 flex xs:p-6 xs:hover:p-8 xs:gap-x-8 md:flex-col md:w-[232px] md:h-[233px] md:hover:h-[257px] md:items-center md:gap-y-4 lg:w-[270px] lg:h-[300px] lg:hover:h-[324px] lg:hover:w-[292px]">
      <div className="relative w-[87px] h-[87px]">
        <Avatar>
          <AvatarFallback className="rounded-full overflow-hidden w-full h-full object-cover bg-primary-100">
            <Image src={LogoDefault} alt={`Logo da equipe ${props.name}`} />
          </AvatarFallback>
          <AvatarImage
            src={props.logoUrl || props.links.banner}
            className="rounded-full w-full h-full object-cover bg-primary-100"
            alt={`Logo da equipe ${props.name}`}
          />
        </Avatar>
      </div>
      <div className="flex flex-col gap-y-6 items-center">
        <span className="text-black text-16 font-bold md:text-18 lg:text-24">
          {props.name}
        </span>
        <ProjectOverlay
          variant="current"
          ...
          videoURL={ytURL}
          overlayTrigger={
            <Button variant="primary" className="rounded-3xl text-nowrap">
              {locale === "en" ? "Learn about" : "Saiba Mais"}{" "}
            </Button>
          }
        >
          {childrenData}
